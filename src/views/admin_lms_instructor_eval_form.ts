...
<body class="bg-slate-50 overflow-hidden">
                ...
                <main class="max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
                    <div class="bg-white rounded-[2.5rem] border border-slate-200/60 shadow-sm overflow-hidden">
                        <div class="p-6 md:p-10">
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-bold text-slate-700 mb-1">총점</label>
                                <input type="number" id="totalScore" min="15" max="75" readonly class="w-24 px-3 py-2 border border-slate-200 rounded-lg bg-slate-50 font-bold text-slate-800">
                                <span class="ml-2 text-slate-600">점</span>
                            </div>
    ...
    <script>
        (function(){
            ...
            fetch('/api/instructor-eval/by-params?session_id='+encodeURIComponent(courseId)+'&subject_name='+encodeURIComponent(subject)+'&evaluator_type='+encodeURIComponent(evaluator), { headers: { 'Authorization': 'Bearer '+token } })
                ...
                .then(function(res){
                    if(res&&res.success&&res.data){
                        ...
                        for(var i=1;i<=15;i++){
                            var v = d['q'+i];
                            if(v!=null){ var radio = document.querySelector('input[name="q'+i+'"][value="'+v+'"]'); if(radio) radio.checked = true; }
                        }
