...
<body class="bg-gray-50 font-sans">
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="px-8 py-6 border-b border-gray-50 flex justify-between items-center bg-white/50 backdrop-blur-md">
                            <div class="flex items-center gap-4">
                                <h3 class="font-black text-gray-800 text-lg uppercase tracking-tight">과정별 설문 참여 현황</h3>
                                <div class="flex bg-gray-100 p-1 rounded-xl">
                                    ...
                                    <button onclick="filterCourses('active')" class="px-4 py-1.5 rounded-lg text-xs font-bold transition-all filter-btn text-gray-500 hover:text-gray-700" data-filter="active">설문중</button>
                                </div>
                            </div>
                            <div class="flex items-center gap-4">
    ...
    <script>
        ...
        function updateStats(data) {
            ...
            const totalSurveys = data.reduce((acc, cur) => acc + (cur.survey_count || 0), 0);
            const totalResponses = data.reduce((acc, cur) => acc + (cur.response_count || 0), 0);
            const studentCount = data.reduce((acc, cur) => acc + (cur.student_count || 0), 0);
            
            // 글로벌 참여율: 총 응답수 / (과정별 설문수 * 학생수)합계
            const totalPossibleResponses = data.reduce((acc, cur) => acc + (cur.survey_count * cur.student_count), 0);
            const globalRate = totalPossibleResponses > 0 ? Math.round((totalResponses / totalPossibleResponses) * 100) : 0;

            document.getElementById('statTotalSurveys').textContent = totalSurveys + '건';
            document.getElementById('statGlobalRate').textContent = globalRate + '%';
            document.getElementById('statTotalResponses').textContent = totalResponses + '명';
            document.getElementById('statTotalTarget').textContent = studentCount + '명';
        }
        ...
        function filterCourses(filter) {
            ...
            if (filter === 'active') {
                filtered = allData.filter(c => c.survey_count > 0);
            }
        ...
        function sortData() {
            ...
            if (sortBy === 'name') {
                ...
                sorted.sort((a, b) => b.survey_count - a.survey_count);
            } else if (sortBy === 'rate') {
        ...
        function renderSummaryTable(data) {
            ...
            tbody.innerHTML = data.map(c => \`
                <tr class="hover:bg-indigo-50/30 transition-colors group border-b border-gray-50 last:border-0 shadow-[inset_0_1px_0_0_rgba(255,255,255,1)]">
                    ...
                    <td class="px-6 py-5 text-center text-sm font-black text-slate-500">
                        \${c.student_count}명
                    </td>
                    <td class="px-6 py-5 text-center">
                        <div class="flex flex-col items-center">
                            <div class="text-[11px] font-black \${c.participation_rate >= 80 ? 'text-emerald-500' : c.participation_rate >= 50 ? 'text-indigo-500' : 'text-amber-500'} italic font-mono uppercase tracking-tighter">\${c.participation_rate}%</div>
