...
<body class="bg-gray-50">
    ...
    <script>
        ...
        function openModal(post) {
            ...
            document.getElementById('multiVideoProgress').textContent = '';
            ...
            if (post) {
                ...
                // 비디오 로드
                if (post.videos) {
                    const vList = Array.isArray(post.videos) ? post.videos : (typeof post.videos === 'string' ? JSON.parse(post.videos) : []);
                    vList.forEach(v => {
                        if (v.startsWith('http') && (v.includes('youtube.com') || v.includes('youtu.be') || v.includes('vimeo.com'))) {
                            videoLinks.push(v);
                        } else {
                            multiUploadedVideos.push(v);
                        }
                    });
                    renderVideoLinks();
        ...
        function setupMultiVideoUpload() {
            ...
            if (!dropZone || !input) return;
            dropZone.addEventListener('click', function(e) { if (!e.target.closest('#multiVideoList')) input.click(); });
