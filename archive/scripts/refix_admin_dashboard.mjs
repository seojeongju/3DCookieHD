...
// Corrected renderPendingList function - DOUBLE ESCAPED BACKSLASHES
...
// Corrected renderAbnormalList function - DOUBLE ESCAPED BACKSLASHES
...
// Replace function logic: Find start, find matching closing brace.
function replaceFunction(source, funcName, newCode) {
    ...
    if (endIdx !== -1) {
        ...
        return source.substring(0, startIdx) + newCode + source.substring(endIdx);
    } else {
        console.log(`Coult not find end of function ${funcName}`);
        return source;
    }
}

content = replaceFunction(content, 'renderPendingList', newPendingList);
content = replaceFunction(content, 'renderAbnormalList', newAbnormalList);
