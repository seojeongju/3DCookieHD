...
- 검색: `GET /api/ncs/search?keyword={검색어}`  
  선택: `ncsLclasCd` — 대분류코드 2자리(예: 01, 15). 미지정 시 01.  
  예: `GET /api/ncs/search?keyword=3D&ncsLclasCd=15`
...
## 참고사항
- **코드 연동**: `NCS_API_KEY`가 설정되면 `GET /api/ncs/search`에서 공공데이터 **NCS 훈련과정정보**(B490007) API를 호출합니다. 키가 없거나 호출 실패 시 Mock 데이터를 반환합니다.
- NCS 훈련과정정보 API는 공공데이터포털에서 "한국산업인력공단_NCS 훈련과정정보"(데이터셋 15086447) 활용신청이 필요할 수 있습니다. 동일 인증키로 사용 가능한지 포털에서 확인하세요.
