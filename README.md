...
- ✅ 빠른 메뉴 네비게이션 (시간표, 위치안내, 포트폴리오, 수강후기, 게시판, 수강료조회)
- ✅ 특징 섹션
- ✅ 수강후기 섹션 (수직 스크롤)
- ✅ 빠른 수강료 조회 폼
- ✅ 시제품 제작 갤러리 (1x5 가로 스크롤)
- ✅ 수강생 포트폴리오 갤러리 (1x5 가로 스크롤)
- ✅ 푸터
...
6. **posts** - 게시글
7. **comments** - 댓글
8. **schedules** - 개강 일정 (신규 추가!)
9. **post_likes** - 게시글 좋아요
10. **review_helpful** - 후기 도움됨
...
### 스케줄 (신규!):
- `GET /api/schedules` - 스케줄 목록 (필터링 지원)
- `GET /api/schedules/:id` - 스케줄 상세
- `GET /api/schedules/calendar/:year/:month` - 월별 달력 데이터
- `POST /api/schedules` - 스케줄 생성 (관리자)
- `PUT /api/schedules/:id` - 스케줄 수정 (관리자)
- `DELETE /api/schedules/:id` - 스케줄 삭제 (관리자)
